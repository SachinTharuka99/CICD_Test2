...
public class MerchantPaymentFileRepo implements MerchantPaymentFileDao {
    ...
    public HashMap<String, String> getCurrencyList() throws Exception {
        ...
        try {
            String sql = "SELECT DISTINCT CURRENCYNUMCODE,CURRENCYALPHACODE FROM CURRENCY WHERE STATUS = ? ";
    ...
    public HashMap<String, HashMap<Integer, HashMap<String, ArrayList<MerchantPaymentCycleBean>>>> getMerchantsForPayment() throws Exception {
        ...
        HashMap<String, HashMap<Integer, HashMap<String, ArrayList<MerchantPaymentCycleBean>>>> merchantListOnPayMode = new HashMap<>();
        //HashMap<String, HashMap<Integer, HashMap<String, ArrayList<MerchantPaymentCycleBean>>>> tempmerchantListOnPayMode = new HashMap<>();
        SimpleDateFormat sdf = new SimpleDateFormat("dd-MM-yy");
        try {
            String query = "SELECT(CASE WHEN MC.PAYMENTMAINTEINANCESTATUS='" + Configurations.YES_STATUS + "' THEN 1 ELSE 0 END) AS PAYSTATUS,MC.MERCHANTCUSTOMERNO AS MERCHANTCUSTOMERNO, ML.MERCHANTID AS MERCHANTID,(CASE WHEN MC.PAYMENTMAINTEINANCESTATUS='" + Configurations.YES_STATUS + "' THEN MC.NEXTPAYEMENTDATE ELSE ML.NEXTPAYEMENTDATE END) AS NEXTPAYEMENTDATE,(CASE WHEN MC.PAYMENTMAINTEINANCESTATUS='" + Configurations.YES_STATUS + "' THEN MC.PAYMENTMODE ELSE ML.PAYMENTMODE END) AS PAYMENTMODE,(CASE WHEN MC.PAYMENTMAINTEINANCESTATUS='" + Configurations.YES_STATUS + "' THEN MC.PAYMENTCYCLE ELSE ML.PAYMENTCYCLE END) AS PAYMENTCYCLE,(CASE WHEN MC.PAYMENTMAINTEINANCESTATUS='" + Configurations.YES_STATUS + "' THEN MC.ACCOUNTNUMBER ELSE ML.ACCOUNTNUMBER END) AS ACCOUNTNO,(CASE WHEN MC.PAYMENTMAINTEINANCESTATUS='" + Configurations.YES_STATUS + "' THEN MC.ACCOUNTNAME ELSE ML.ACCOUNTNAME END) AS ACCOUNTNAME,(CASE WHEN MC.PAYMENTMAINTEINANCESTATUS='" + Configurations.YES_STATUS + "' THEN MC.BANKCODE ELSE ML.BANKCODE END) AS BANKCODE,(CASE WHEN MC.PAYMENTMAINTEINANCESTATUS='" + Configurations.YES_STATUS + "' THEN MC.BRANCHNAME ELSE ML.BRANCHNAME END) AS BRANCHNAME,(CASE WHEN MC.PAYMENTMAINTEINANCESTATUS='" + Configurations.YES_STATUS + "' THEN MC.CURRENCYCODE ELSE ML.CURRENCYCODE END) AS CURRENCYCODE, MC.STATUS AS MERCUSSTATUS, ML.STATUS AS MERSTATUS FROM MERCHANTCUSTOMER MC INNER JOIN MERCHANTLOCATION ML ON ML.MERCHANTCUSTOMERNO = MC.MERCHANTCUSTOMERNO WHERE ((CASE WHEN MC.PAYMENTMAINTEINANCESTATUS='" + Configurations.YES_STATUS + "' THEN TRUNC(MC.NEXTPAYEMENTDATE) ELSE TRUNC(ML.NEXTPAYEMENTDATE) END) <= TO_DATE('" + sdf.format(Configurations.EOD_DATE) + "','DD-MM-YY')) AND ((CASE WHEN MC.PAYMENTMAINTEINANCESTATUS='" + Configurations.YES_STATUS + "' THEN MC.STATUS ELSE ML.STATUS END) NOT IN ('" + status.getMERCHANT_DELETE_STATUS() + "', '" + status.getMERCHANT_CANCEL_STATUS() + "')) ";

            if (Configurations.STARTING_EOD_STATUS.equals(status.getINITIAL_STATUS())) {
                query += " AND (CASE WHEN MC.PAYMENTMAINTEINANCESTATUS='YES' AND MC.MERCHANTCUSTOMERNO NOT IN (SELECT EM.MERCHANTCUSTOMERNO FROM EODERRORMERCHANT EM WHERE EM.STATUS='" + status.getEOD_PENDING_STATUS() + "') THEN 1 WHEN MC.PAYMENTMAINTEINANCESTATUS='NO' AND ML.MERCHANTID NOT IN (SELECT EM.MID FROM EODERRORMERCHANT EM WHERE EM.STATUS='" + status.getEOD_PENDING_STATUS() + "') THEN 1 ELSE 0 END) = 1";
            } else if (Configurations.STARTING_EOD_STATUS.equals(status.getERROR_STATUS())) {
                query += " AND (CASE WHEN MC.PAYMENTMAINTEINANCESTATUS='YES' AND MC.MERCHANTCUSTOMERNO IN (SELECT EM.MERCHANTCUSTOMERNO FROM EODERRORMERCHANT EM WHERE EM.STATUS='" + status.getEOD_PENDING_STATUS() + "'AND EODID < " + Configurations.ERROR_EOD_ID + " AND EM.processstepid <=" + Configurations.PROCESS_STEP_ID + ") THEN 1 WHEN MC.PAYMENTMAINTEINANCESTATUS='NO' AND ML.MERCHANTID IN (SELECT EM.MID FROM EODERRORMERCHANT EM WHERE EM.STATUS='" + status.getEOD_PENDING_STATUS() + "' AND EODID < " + Configurations.ERROR_EOD_ID + " AND EM.processstepid <=" + Configurations.PROCESS_STEP_ID + ") THEN 1 ELSE 0 END) = 1";
            }
            ...
            backendJdbcTemplate.query(query,
                    (ResultSet rs) -> {
                        ...
                        while (rs.next()) {
                            ...
                            merchantPaymentCycleBean.setMerchantId(rs.getString("MERCHANTID"));
                            merchantCustomer = rs.getString("MERCHANTCUSTOMERNO");
                            ...
                            merchantPaymentCycleBean.setPaymentCycleCode(rs.getString("PAYMENTCYCLE"));
                            merchantPaymentCycleBean.setMerchantStatus(rs.getString("MERSTATUS"));
                            merchantPaymentCycleBean.setMerchantCustomerStatus(rs.getString("MERCUSSTATUS"));
                            merchantPaymentCycleBean.setAccountNo(rs.getString("ACCOUNTNO"));
                            merchantPaymentCycleBean.setAccountName(rs.getString("ACCOUNTNAME"));
                            merchantPaymentCycleBean.setBankCode(rs.getString("BANKCODE"));
                            merchantPaymentCycleBean.setBranchCode(rs.getString("BRANCHNAME"));
                            merchantPaymentCycleBean.setCurrencyCode(rs.getString("CURRENCYCODE"));
...
//            SimpleDateFormat sdf = new SimpleDateFormat("dd-MMM-yy");
            ...
            String query = "SELECT ML.MERCHANTCUSTOMERNO,ML.MERCHANTID,CASE WHEN MBC.PAYMENTOPTION = '1' THEN TO_DATE(ML.NEXTPAYEMENTDATE) + 1 WHEN MBC.PAYMENTOPTION = '2' THEN TO_DATE(ML.NEXTPAYEMENTDATE) + 7 WHEN MBC.PAYMENTOPTION = '3'THEN LAST_DAY((ML.NEXTPAYEMENTDATE)) + MBC.PAYMENTDATE WHEN MBC.PAYMENTOPTION = '4' THEN TO_DATE(ML.NEXTPAYEMENTDATE) + MBC.NOOFDAYS END AS NEWNEXTPAYEMENTDATE, ML.NEXTPAYEMENTDATE, MBC.PAYMENTOPTION, MBC.PAYMENTDATE, MBC.NOOFDAYS FROM MERCHANTLOCATION ML INNER JOIN MERCHANTPAYMENTCYCLE MBC ON ML.PAYMENTCYCLE = MBC.PAYMENTCYCLECODE WHERE ML.MERCHANTID= ?";
            ...
            for (String cusNo : merchantList) {
                ...
                if (!nextBillingDate.equals(null)) {
                    ...
                    String updateQuery = "UPDATE MERCHANTLOCATION SET NEXTPAYEMENTDATE = TO_DATE(?,'DD-MM-YY') WHERE MERCHANTID = ? ";
...
//            SimpleDateFormat sdf = new SimpleDateFormat("dd-MMM-yy");
            ...
            String query = "SELECT MC.MERCHANTCUSTOMERNO, CASE WHEN MBC.PAYMENTOPTION = '1' THEN TO_DATE(MC.NEXTPAYEMENTDATE) + 1 WHEN MBC.PAYMENTOPTION = '2' THEN TO_DATE(MC.NEXTPAYEMENTDATE) + 7 WHEN MBC.PAYMENTOPTION = '3' THEN LAST_DAY((MC.NEXTPAYEMENTDATE)) + MBC.PAYMENTDATE WHEN MBC.PAYMENTOPTION = '4' THEN TO_DATE(MC.NEXTPAYEMENTDATE) + MBC.NOOFDAYS END AS NEWNEXTPAYEMENTDATE, MC.NEXTPAYEMENTDATE, MBC.PAYMENTOPTION, MBC.PAYMENTDATE, MBC.NOOFDAYS FROM MERCHANTCUSTOMER MC INNER JOIN MERCHANTPAYMENTCYCLE MBC ON MC.PAYMENTCYCLE = MBC.PAYMENTCYCLECODE WHERE MC.MERCHANTCUSTOMERNO= ?";
            ...
            for (String cusNo : merchantList) {
                ...
                if (!nextBillingDate.equals(null)) {
                    ...
                    String updateQuery = "UPDATE MERCHANTCUSTOMER SET NEXTPAYEMENTDATE = TO_DATE(?,'DD-MM-YY') WHERE MERCHANTCUSTOMERNO = ? ";
...
//        ArrayList<String> glTypes = new ArrayList<String>();
        ...
        try {
            String sql = "SELECT EODPAYID,MERCHANTCUSTID,MERCHANTID,NETPAYAMMOUNT,CRDRNET FROM EODMERCHANTPAYMENT WHERE PAYMENTFILESTATUS = ? AND MERCHANTCUSTID = ?";
...
//        ArrayList<String> glTypes = new ArrayList<String>();
        ...
        try {
            String sql = "SELECT EODPAYID,MERCHANTID,NETPAYAMMOUNT,CRDRNET FROM EODMERCHANTPAYMENT WHERE PAYMENTFILESTATUS = ? AND MERCHANTID = ?";
    ...
    public int InsertMerchantPaymentFilesIntoDownloadTable(String fileId, String fileType) throws Exception {
        int count;
        try {
            String query = "Insert into DOWNLOADFILE (FIETYPE,FILENAME,LETTERTYPE, STATUS,GENERATEDUSER,STATEMENTMONTH,STATEMENTYEAR,LASTUPDATEDTIME, CREATEDTIME,LASTUPDATEDUSER,CARDTYPE,CARDPRODUCT,FILEID ) values (?,?,?,?,?,?,?,to_date(?,'DD-MM-YY'),to_date(?,'DD-MM-YY'),?,?,?,?)";
