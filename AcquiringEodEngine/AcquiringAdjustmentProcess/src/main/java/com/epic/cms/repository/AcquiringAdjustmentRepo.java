...
public class AcquiringAdjustmentRepo implements AcquiringAdjustmentDao {
    ...
    public ArrayList<AcqAdjustmentBean> getConfirmedAjustments() throws Exception {
        ...
        try {
            ...
            query = "SELECT REQUESTID ,CARDNUMBER,MERCHANTID,"
                    + " CARDORMERCHANT ,"
                    + " TXNID ,"
                    + " AMOUNT ,"
                    + " REMARKS,"
                    + " CRDR ,"
                    + " MCC,"
                    + " CURRENCYTYPE,ADJUSTDATE,"
                    + " TRANSACTIONTYPE ,"
                    + " ADJUSTMENTTYPE"
                    + " FROM ACQADJUSTMENT"
                    + " WHERE EODSTATUS = ?"
                    + " AND TRUNC(ADJUSTDATE) <= TO_DATE(?,'DD-MM-YY')"
                    + " AND STATUS      = ?";
...
//                            adjustmentBean.setCardOrMerchant(cardOrMerchant);
    ...
    public int insertToEodMerchantPayment(MerchantPayBean merchantPaymentBean, String adjustmentFlag) throws Exception {
        ...
        try {
            query = "INSERT INTO EODMERCHANTPAYMENT "
                    + "(EODID,MERCHANTID,MERCHANTCUSTID,MERACCOUNTNO,CUSTACCOUNTNO,TOTALPAYAMOUNT,CRDR,"
                    + "CURRENCYTYPE,TOTALFEEAMOUNT,CRDRFEE,TOTALCOMMISSION,CRDRCOMMISSION,NETPAYAMMOUNT,"
                    + "CRDRNET,TOACCOUNTNO,STATUS,LASTUPDATEDTIME,CREATEDTIME,LASTUPDATEDUSER,PAYMENTDATE,TXNCOUNT,ADJUSTMENTFLAG)"
    ...
    public int insertToEodMerchantComission(String merchantCusNo, String merhantCusAcountNo, String mId, String merchantAccNo, String tId, String txnAmount, String merchantComission, String currency, String crDr, Date txnDate, String txnTypeCode, String batchNo, String txnId, String binStatus, String calMethod, String cardAssociation, String cardProduct, String segment, String originCardProduct, int adjustmentFlag) throws Exception {
        ...
        try {
            query = "INSERT INTO EODMERCHANTCOMMISSION (EODID,MERCHANTCUSTID,CUSTACCOUNTNO,MID,MERACCOUNTNO,"
                    ...
                    + "LASTUPDATEDTIME,STATUS,BINTYPE,CALMETHOD,CARDASSOCIATION,PRODUCTID,SEGMENT,EODDATE,CARDPRODUCT,ADJUSTMENTFLAG) VALUES (?,?,?,?,?,?,?,?,?,?,?,TO_DATE(?,'DD-MM-YY'),?,?,?,?,SYSDATE,SYSDATE,?,?,?,?,?,?,TO_DATE(?,'DD-MM-YY'),?,?)";
    ...
    public void insertToEODMerchantFee(MerchantFeeBean merchantFeeBean, String amount, java.util.Date effectDate) throws Exception {
        ...
        try {
            query = "INSERT INTO EODMERCHANTFEE(EODID,MERCHANTID,CRDR,EFFECTDATE,FEEAMOUNT,CUSTACCOUNTNO,MERACCOUNTNO,MERCHANTCUSTID,"
                    + "STATUS,FEETYPE,LASTUPDATEDUSER,LASTUPDATEDDATE)"
                    + "VALUES (?,?,?,?,?,?,?,?,?,?,?,SYSDATE)";
...
//        return false;
    ...
    public int insertReversalTxnIntoTxnTable(String oldTxnId, String newTxnID) throws Exception {
        ...
        try {
            query = "INSERT INTO TRANSACTION(TXNID,MTI,RESPONSEMTI,PROCESSINGCODE,OFFMTI,OFFRESPONSEMTI,OFFPROCESSINGCODE,NII"
                    + ",TXNTYPECODE,BIN,REQUESTFROM,LISTENERTYPE,ONOFFSTSTUS,SERVICECODE,CARDNO,EXPIRYDATE,TXNCURRENCY,TXNAMOUNT,BILLINGCURRENCY"
                    + ",BILLINGAMOUNT,SETTLEMENTCURRENCY,SETTLEMENTAMOUNT,SETTLEMENTTXNCOUNT,SETTLEMENTDATE,BATCHNO,TID,MID"
                    + ",MCC,COUNTRYCODE,TRACENO,INVOICENO,RRN,AUTHCODE,RESPONSECODE,STATUS,POSCONDITIONCODE,POSENTRYMODE,AIIC"
                    + ",FIIC,TAG5F2A,TAG9A,TAG9C,TAG9F34,TAG9F02,TAG9F03,TAG9F1A,TAG9F1E,TAG9F27,TAG9F33,TAG9F35,TAG9F41"
                    + ",TXNTIME,TXNDATE,LOCALTIME,LOCALDATE,CREATETIME,LASTUPDATETIME,CARDSEQUENSENUMBER,FROMACCOUNT,TOACCOUNT"
                    + ",CHANNELTYPE,EODSTATUS,ACCEPTORNAME,BILLACCOUNTNO,BILLPROVIDERID"
                    + ",BILLREFNO,CAIC,F60TERMTYPE,F62TXNID,TXNSUBTYPE,CHQBANKNAME,CHQBRANCHNAME"
                    + ",CHQNO,CHQRETURNDATE,PAYMENTINITTYPE,PAYMENTMODE,VISATXNSTATUS,BACKENDTXNTYPE,AUTOSETTLEMENTSTATUS"
                    + ",ONLINECREATEDTIME,CB_SEQ_NO,ECI,REMARKS,ORIGINALTXNID,"
                    + "CVM,VISACVV2RESULT,VISAREQRESREASONCODE,EMV_9F33,EMV_95,EMV_82,EMV_9A,EMV_9C,EMV_5F2A,EMV_9F02,EMV_9F03,"
                    + "EMV_9F10,EMV_9F1A,EMV_9F26,EMV_9F36,EMV_9F37,EMV_9F6E,LISTENERID,PURCHASE_ID,SECOND_PARTY_PAN,ACQORISS,EODCONSIDERSTATUS,MVV,EMV_9F27,F60_DATA,F15_SETTLE_DATE,TXNTRANDATEANDTIME) "
                    + " SELECT ?,MTI,RESPONSEMTI,PROCESSINGCODE,OFFMTI,OFFRESPONSEMTI,OFFPROCESSINGCODE,NII,?,BIN"
                    + ",REQUESTFROM,LISTENERTYPE,ONOFFSTSTUS,SERVICECODE,CARDNO,EXPIRYDATE,TXNCURRENCY,TXNAMOUNT,BILLINGCURRENCY"
                    + ",BILLINGAMOUNT,SETTLEMENTCURRENCY,SETTLEMENTAMOUNT,SETTLEMENTTXNCOUNT,SETTLEMENTDATE,BATCHNO,TID,MID"
                    + ",MCC,COUNTRYCODE,NULL,INVOICENO,RRN,AUTHCODE,RESPONSECODE,STATUS,POSCONDITIONCODE,POSENTRYMODE,AIIC"
                    + ",FIIC,TAG5F2A,TAG9A,TAG9C,TAG9F34,TAG9F02,TAG9F03,TAG9F1A,TAG9F1E,TAG9F27,TAG9F33,TAG9F35,TAG9F41,TXNTIME"
                    + ",TXNDATE,LOCALTIME,LOCALDATE,CREATETIME,LASTUPDATETIME,CARDSEQUENSENUMBER,FROMACCOUNT,TOACCOUNT,CHANNELTYPE"
                    + ",?,ACCEPTORNAME,BILLACCOUNTNO,BILLPROVIDERID,BILLREFNO,CAIC,F60TERMTYPE,F62TXNID,TXNSUBTYPE,CHQBANKNAME"
                    + ",CHQBRANCHNAME,CHQNO,CHQRETURNDATE,PAYMENTINITTYPE,PAYMENTMODE,VISATXNSTATUS,?,AUTOSETTLEMENTSTATUS"
                    + ",ONLINECREATEDTIME,CB_SEQ_NO,ECI,REMARKS"
                    + ",?,CVM,VISACVV2RESULT,VISAREQRESREASONCODE,EMV_9F33,EMV_95,EMV_82,EMV_9A,EMV_9C,EMV_5F2A,EMV_9F02,"
                    + "EMV_9F03,EMV_9F10,EMV_9F1A,EMV_9F26,EMV_9F36,EMV_9F37,EMV_9F6E,LISTENERID,PURCHASE_ID,SECOND_PARTY_PAN,"
                    + "ACQORISS,EODCONSIDERSTATUS,MVV,EMV_9F27,F60_DATA,F15_SETTLE_DATE,TO_CHAR(SYSDATE,'MMDDHHMISS') FROM TRANSACTION WHERE TXNID=?";
    ...
    public int insertReversalTxnIntoMerchantTxnTable(String oldTxnId, String newTxnID) throws Exception {
        ...
        SimpleDateFormat sdf = new SimpleDateFormat("dd-MMM-yy");
        try {
            String query = "INSERT INTO EODMERCHANTTRANSACTION ("
                    + " EODID,MID,TID,TRANSACTIONAMOUNT,CURRENCYTYPE,CRDR,SETTLEMENTDATE,TRANSACTIONDATE,TRANSACTIONTYPE"
                    + " ,BATCHNO,TRANSACTIONID,LASTUPDATEDUSER,CREATEDTIME,LASTUPDATEDTIME,TOACCOUNTNO,"
                    + " STATUS,TRANSACTIONDESCRIPTION,COUNTRYNUMCODE,ONOFFSTATUS,PAYMENTTYPE,POSENTRYMODE,"
                    + " SEQUENCENUMBER,TRACEID,FOREXMARKUPAMOUNT,AUTHCODE,RRN,MCC,OUTGOINGFILESTATUS,CARDNUMBER,BIN,ADJUSTMENTFLAG,CARDASSOCIATION,REQUESTFROM,CARDPRODUCT) "
                    + " SELECT "
                    + " ?,MID,TID,TRANSACTIONAMOUNT,CURRENCYTYPE,?,?,TRANSACTIONDATE,?"
                    + " ,BATCHNO,?,?,SYSDATE,SYSDATE,TOACCOUNTNO"
                    + " ,?,TRANSACTIONDESCRIPTION ,COUNTRYNUMCODE,ONOFFSTATUS,PAYMENTTYPE,POSENTRYMODE"
                    + " ,SEQUENCENUMBER,TRACEID,FOREXMARKUPAMOUNT,AUTHCODE,RRN,MCC,'0',CARDNUMBER,BIN,?,CARDASSOCIATION,REQUESTFROM,CARDPRODUCT "
                    + " FROM EODMERCHANTTRANSACTION WHERE TRANSACTIONID=?";
    ...
    public int insertReversalCommission(String oldTxnId, String newTxnID) throws Exception {
        ...
        SimpleDateFormat sdf = new SimpleDateFormat("dd-MMM-yy");
        try {
            query = "INSERT INTO EODMERCHANTCOMMISSION ("
                    + " EODID,MERCHANTCUSTID,CUSTACCOUNTNO,MID,MERACCOUNTNO,TID,TRANSACTIONAMOUNT,MERCHANTCOMMSSION,MERCHANTDUEAMOUNT,CURRENCYTYPE"
                    + " ,CRDR,TRANSACTIONDATE,TRANSACTIONTYPE,BATCHNO,TRANSACTIONID,LASTUPDATEDUSER,CREATEDTIME,LASTUPDATEDTIME,TOACCOUNTNO"
                    + " ,STATUS,BINTYPE,CALMETHOD,CARDASSOCIATION,PRODUCTID,SEGMENT,BINSTATUS,EODDATE,ADJUSTMENTFLAG,CARDPRODUCT,MDRFLATAMOUNT,MDRPERCENTAGE ) "
                    + " SELECT "
                    + " ?,MERCHANTCUSTID,CUSTACCOUNTNO,MID,MERACCOUNTNO,TID,TRANSACTIONAMOUNT,MERCHANTCOMMSSION,MERCHANTDUEAMOUNT,CURRENCYTYPE"
                    + " ,?,?,?,BATCHNO,?,?,SYSDATE,SYSDATE,TOACCOUNTNO"
                    + " ,?,BINTYPE,CALMETHOD,CARDASSOCIATION,PRODUCTID,SEGMENT,BINSTATUS,?,?,CARDPRODUCT,MDRFLATAMOUNT,MDRPERCENTAGE "
                    + " FROM EODMERCHANTCOMMISSION WHERE TRANSACTIONID=?";
    ...
    public MerchantDetailsBean getMerchanDetails(MerchantDetailsBean merchantDetailsBean) throws Exception {
        ...
        String query = "SELECT MC.MERCHANTCUSTOMERNO AS MERCHANTCUSTOMERNO,MC.ACCOUNTNUMBER AS MCACCOUNTNO,ML.ACCOUNTNUMBER AS MLACCOUNTNO,ML.COUNTRY AS MLCOUNTRY "
...
//            merchantDetailsBean =
    ...
    public int insertToEODTransaction(StringBuffer cardnumber, String accountNo, String mId, String tId, String txnAmount, int currencyType, String crDr, java.util.Date settlementDate, java.util.Date txnDate, String txnType, String batchNo, String txnId, String toAccNo, Double loyaltyPoint, String Description, String countryCode, int onOffStatus, String poStringsEntryMode, String traceId, String authCode, int adjustemntFlag, String cardAssociation) throws Exception {
        ...
        try {
             ...
             * TODO Loyalty point??
             */
            String query = "INSERT INTO EODTRANSACTION (EODID,CARDNUMBER,ACCOUNTNO,MID,TID,TRANSACTIONAMOUNT,"
                    + " CURRENCYTYPE,CRDR,SETTLEMENTDATE,TRANSACTIONDATE,TRANSACTIONTYPE,BATCHNO,"
                    + " TRANSACTIONID,LASTUPDATEDUSER,CREATEDTIME,LASTUPDATEDTIME,TOACCOUNTNO,STATUS,TRANSACTIONDESCRIPTION,"
                    + " COUNTRYNUMCODE,ONOFFSTATUS,POSENTRYMODE,TRACEID,AUTHCODE,ACQADJUSTMENTFLAG,CARDASSOCIATION)"
                    + " VALUES (?,?,?,?,?,?,?,?,TO_DATE(?,'DD-MM-YY'),TO_DATE(?,'DD-MM-YY'),?,?,?,?,SYSDATE,SYSDATE,?,?,?,?,?,?,?,?,?,?)";
    ...
    public int insertReversalTxnIntoTxnTable(AcqAdjustmentBean bean, String newTxnID, int onOffStatus) throws Exception {
        ...
        int count = 0;
        try {
            query = "INSERT INTO TRANSACTION(TXNID,MTI,RESPONSEMTI,PROCESSINGCODE"
                    + ",TXNTYPECODE,BIN,REQUESTFROM,LISTENERTYPE,ONOFFSTSTUS,CARDNO,TXNCURRENCY,TXNAMOUNT,BILLINGCURRENCY"
                    + ",BILLINGAMOUNT,SETTLEMENTTXNCOUNT,TID,MID"
                    + ",MCC,RRN,AUTHCODE,RESPONSECODE,STATUS,POSCONDITIONCODE"
                    + ",TXNDATE,CREATETIME,LASTUPDATETIME"
                    + ",CHANNELTYPE,EODSTATUS"
                    + ",CAIC,BACKENDTXNTYPE,AUTOSETTLEMENTSTATUS"
                    + ",ONLINECREATEDTIME,ORIGINALTXNID,AIIC,FIIC,TXNTRANDATEANDTIME) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,SYSDATE,SYSDATE,?,?,?,?,?,SYSDATE,?,?,?,TO_CHAR(SYSDATE,'MMDDHHMISS')) ";
            int channelType;
            String aiic = "";
            String fiic = "";
    ...
    public int insertIntoEodMerchantTransaction(EodTransactionBean eodTransactionBean, String status) throws Exception {
        int count = 0;
        try {
            String query = "INSERT INTO EODMERCHANTTRANSACTION (AUTHCODE,BATCHNO,COUNTRYNUMCODE,CRDR,CREATEDTIME,"
                    + "CURRENCYTYPE,EODID,FOREXMARKUPAMOUNT,GLSTATUS,LASTUPDATEDTIME,LASTUPDATEDUSER,MID,ONOFFSTATUS,"
                    + "PAYMENTTYPE,POSENTRYMODE,RRN,SEQUENCENUMBER,SETTLEMENTDATE,STATUS,TID,TOACCOUNTNO,"
                    + "TRACEID,TRANSACTIONAMOUNT,TRANSACTIONDATE,TRANSACTIONDESCRIPTION,TRANSACTIONID,TRANSACTIONTYPE,MCC,"
                    + "BIN,CARDNUMBER,ADJUSTMENTFLAG,REQUESTFROM,FUELSURCHARGEAMOUNT,CARDASSOCIATION,CARDPRODUCT) "
                    + "VALUES(?,?,?,?,SYSDATE,?,?,?,?,SYSDATE,?,?,?,?,?,?,?,?,?,?,?,?,?,TO_DATE(?,'DD-MM-YY'),?,?,?,?,?,?,?,?,?,?,?)";
            SimpleDateFormat sdf = new SimpleDateFormat("dd-MMM-yy");
            count = backendJdbcTemplate.update(query
    ...
    public void getPaymentAmount(String txnId, MerchantPayBean paymentBean) throws Exception {
        ...
        try {
            backendJdbcTemplate.query(query,
                    (ResultSet rs) -> {
                        if (rs.next()) {
                            paymentBean.setNetPayAmount(rs.getString("PAYMENTAMOUNT"));
    ...
    public int setCardProductToEodMerTxn() throws Exception {
        ...
        String query = "MERGE INTO EODMERCHANTTRANSACTION EMT USING"
                + " ( SELECT BINRANGESTART,BINRANGEEND,PRODUCTID FROM BINTABLE) BT ON (EMT.BIN>=BT.BINRANGESTART AND EMT.BIN<=BT.BINRANGEEND)"
                + " WHEN MATCHED THEN"
                + " UPDATE SET EMT.CARDPRODUCT=BT.PRODUCTID WHERE EMT.CARDPRODUCT IS NULL ";
        try {
            count = backendJdbcTemplate.update(query);
