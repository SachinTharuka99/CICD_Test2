...
public class LoyaltyPointsCalculationRepo implements LoyaltyPointsCalculationDao {
    ...
    public double getLastStmtClosingLoyalty(StringBuffer cardNo, String statementId) throws Exception {
        ...
        try {
            ...
            loyaltyPoints = backendJdbcTemplate.queryForObject(sql, Double.class, cardNo.toString(), statementId);
    ...
    public double getThisMonthRedeem(StringBuffer cardNo, Date stmtStartDate, Date stmtEndDate, ArrayList<Integer> requestID) throws Exception {
        ...
        try {
            ...
            loyaltyPoints = backendJdbcTemplate.queryForObject(query, double.class, cardNo.toString(), sdf.format(stmtStartDate), sdf.format(stmtEndDate)
            );
        } catch (Exception e) {
            throw e;
        }
    ...
    public double getAdjustLoyalty(String accNo, Date stmtStartDate, Date stmtEndDate) throws Exception {
        ...
        try {
            adjustLoyalty = backendJdbcTemplate.queryForObject(sql, Double.class, Configurations.CREDIT, Configurations.LOYALTY_ADJUSTMENT_TYPE, accNo, sdf.format(stmtStartDate), sdf.format(stmtEndDate), statusList.getMANUAL_ADJUSTMENT_ACCEPT(), Configurations.DEBIT, Configurations.LOYALTY_ADJUSTMENT_TYPE, accNo, sdf.format(stmtStartDate), sdf.format(stmtEndDate), statusList.getMANUAL_ADJUSTMENT_ACCEPT());
        } catch (Exception e) {
            throw e;
        }
    ...
    public void updateBillingStatment(LoyaltyBean loyaltyBean) throws Exception {
        try {
            ...
            backendJdbcTemplate.update(sql, loyaltyBean.getOpeningLoyaltyPoints(), loyaltyBean.getEarnLoyaltyPoints(), loyaltyBean.getAvailableLoyaltyPoints(), loyaltyBean.getClosingLoyaltyPoints(), loyaltyBean.getRedeemLoyaltyPoints(), loyaltyBean.getAdjustLoyaltyPoints(), loyaltyBean.getStatementId(), loyaltyBean.getCardNo().toString());
        } catch (Exception e) {
            throw e;
        }
    }
    ...
    public int updateLoyaltyRedeemRequest(ArrayList<Integer> requestID, String EOD_DONE_STATUS) throws Exception {
        ...
        try {
            for (Integer reqID : requestID) {
                ...
                count = backendJdbcTemplate.update(query, EOD_DONE_STATUS, eodDate, reqID);
            }
