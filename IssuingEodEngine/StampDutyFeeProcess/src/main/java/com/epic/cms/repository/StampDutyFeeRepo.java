...
public class StampDutyFeeRepo implements StampDutyFeeDao {
    ...
    public double getTotalForeignTxns(String inClauseString, int startEodID) throws Exception {
        ...
        try {
            String query = "SELECT SUM(ET.TRANSACTIONAMOUNT) AS TOTAL FROM EODTRANSACTION ET INNER JOIN CARDACCOUNT CA ON CA.ACCOUNTNO = ET.ACCOUNTNO INNER JOIN INTERESTPROFILETRANSACTION IPT ON CA.INTERESTPROFILECODE = IPT.INTERESTPROFILE AND IPT.TRANSACTIONCODE =  ET.TRANSACTIONTYPE WHERE ET.EODID > ? AND ET.EODID <= ? AND ET.CRDR='DR' AND COUNTRYNUMCODE !=? AND ET.CARDNUMBER IN ( ? )"; //AND ET.CARDNUMBER IN ( ? )
            ...
            String query1 = "SELECT SUM(ET.TRANSACTIONAMOUNT) AS TOTAL FROM EODTRANSACTION ET INNER JOIN CARDACCOUNT CA ON CA.ACCOUNTNO = ET.ACCOUNTNO INNER JOIN INTERESTPROFILETRANSACTION IPT ON CA.INTERESTPROFILECODE = IPT.INTERESTPROFILE AND IPT.TRANSACTIONCODE =  ET.TRANSACTIONTYPE WHERE ET.EODID > ? AND ET.EODID <= ? AND ET.CRDR='CR' AND COUNTRYNUMCODE !=? AND ET.CARDNUMBER IN ( ? )"; //AND ET.CARDNUMBER IN ( ? )
